...
def search_records(query, top_k=5):
    ...
    # Set n_results to the minimum of top_k or the total number of elements
    n_results = min(top_k, total_elements)

    # Perform similarity search using ChromaDB
    results = collection.query(
        query_embeddings=[query_embedding],
        n_results=n_results,  # Limit to top_k results
...
def generate_chatgpt_answer(query, top_records):
    """
    Generates an answer to the user's query using ChatGPT, based on the top_records.

    :param query: The user's search query.
    :param top_records: A list of the top similar records (dictionaries).
    :return: The answer generated by ChatGPT.
    """
    # Format the top records into a context string with detailed information
    context = "\n\n".join([
        f"""
        Record {i+1}:
        - Minit Number: {record['metadata'].get('minit_number', 'Unknown')}
        - Summary: {record['metadata'].get('summary', 'N/A')}
        - Tag: {record['metadata'].get('tag', 'N/A')}
        - Impact: {record['metadata'].get('impact', 'N/A')}
        - Date: {record['metadata'].get('date', 'Unknown')}
        - PDF File: {record['metadata'].get('pdf_name', 'Unknown')}
        - Source: The above information was retrieved from minute {record['metadata'].get('minit_number', 'Unknown')} in the PDF file '{record['metadata'].get('pdf_name', 'Unknown')}'.
        """
    ...
    Here are the top 5 relevant records from our database:

    {context}

    Using the information from the above records, please answer the following question:

    "{query}"

    Provide a concise and informative response based solely on the provided records. Mention Source of answer including the pdf name and minit as well.
    """
